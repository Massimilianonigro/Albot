...
class StateMachine:
    ...
    def input_function(self,intent,current_state):
        print("In state_machine input_function: " + str(current_state))
    ...
    def greeting(self,intent,current_state): 
        ...
        if intent['intent']['name'] == "connected" :
            utterance_array.append(rand.choice(self.utterances['greeting']))
            utterance_array.append(rand.choice(self.utterances['introduction_or_practice']))
        elif intent['intent']['name'] == "clicked_practice":
            next_state = State.PRACTICE_COLLECTING
            utterance_array.append(rand.choice(self.utterances['practice_explanation']))
        elif intent['intent']['name'] == "clicked_introduction":
            next_state = State.INTRODUCTION_START
            utterance_array.append(rand.choice(self.utterances['introduction_explanation']))
        elif intent['intent']['name'] == "nlu_fallback":
            utterance_array.append(rand.choice(self.utterances['fallback']))
        return next_state , utterance_array
    ...
    def introduction_start(self,intent,current_state):
        ...
        if question_explanation != None:
            utterance_array.append(rand.choice(question_explanation))
        elif intent['intent']['name'] == "nlu_fallback":
            utterance_array.append(rand.choice(self.utterances['fallback']))
        return next_state,utterance_array
    ...
    def acid_selection(self,intent,current_state):
        ...
        if intent['intent']['name'] == "clicked_next":
            utterance_array.append(rand.choice(self.utterances['base_selection']))
            next_state = State.BASE_SELECTION
        elif intent['intent']['name'] == "nlu_fallback":
            utterance_array.append(rand.choice(self.utterances['fallback'].choice))
        return next_state,utterance_array
    ...
    def base_selection(self,intent,current_state):
        ...
        if question_explanation != None:
            ...
            return next_state,utterance_array
        if intent['intent']['name'] == "clicked_next":
            utterance_array.append(rand.choice(self.utterances['guided_reasoning_explanation']))
    ...
    def guided_reasoning(self,intent,current_state):
        ...
        if intent['intent']['name'] == "chatbot_question":
            utterance_array.append(rand.choice(self.utterances['chatbot_question'])) 
        elif intent['intent']['name'] == "nlu_fallback":
            utterance_array.append(rand.choice(self.utterances['fallback']))
        return next_state, utterance_array
    ...
    def guided_pouring(self,intent,current_state):
        ...
        if intent['intent']['name'] == "chatbot_question":
            ...
            next_state = State.PRACTICE_COLLECTING
            utterance_array.append(rand.choice(self.utterances['practice_collecting_explanation']))
        elif intent['intent']['name'] != "clicked_next" and intent['intent']['name'][0:7] == "clicked":
            utterance_array.append(rand.choice(self.utterances['color_change_exclamation']))
        return next_state, utterance_array
